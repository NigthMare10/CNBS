...
import { detectWorkbookKind } from "../workbooks/signatures";
...
import { canonicalCatalogs, normalizeFinancialPositionFacts, normalizePremiumFacts } from "./normalization";
...
function domainAvailabilityFromInput(input: {
  ...
  return {
    ...
    incomeStatement: {
      ...
      records: 0,
      publishable: false,
      missingReason: "Income statement raw source is not available in phase 1."
    },
    reference: {
...
export class IngestionService {
  ...
  async ingestWorkbookSet(input: {
    ...
    const normalizationSummary: ValidationSummary = {
      publishability: [...validationIssues, ...parsedValidation.issues, ...normalizedPremiums.issues, ...normalizedFinancials.issues].some(
        (issue) => issue.severity === "critical" || issue.severity === "high"
      )
        ? "blocked"
        : [...validationIssues, ...parsedValidation.issues, ...normalizedPremiums.issues, ...normalizedFinancials.issues].some(
              (issue) => issue.severity === "medium"
            )
          ? "warningOnly"
          : "publishable",
      issues: [...validationIssues, ...parsedValidation.issues, ...normalizedPremiums.issues, ...normalizedFinancials.issues]
    };
  ...
  async getPublicOverview() {
    ...
    if (!active) {
      return { metadata: null, executiveKpis: [], premiumsByInstitution: [], premiumsByLine: [], financialHighlights: [] };
    }

    const [executiveKpis, premiumsByInstitution, premiumsByLine, financialHighlights] = await Promise.all([
  ...
  async getPublicInstitutionDetail(institutionId: string) {
    ...
    try {
      return await this.storage.readPublishedArtifact(
        active.datasetVersionId,
        `aggregates/institutions/${institutionId}.json`
      );
    } catch {
      const [premiumFacts, financialFacts, premiumsByInstitution, financialHighlights] = await Promise.all([
